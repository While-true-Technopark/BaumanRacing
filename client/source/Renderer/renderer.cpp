...
renderer::renderer(sf::RenderWindow* win) {
    ...
    view = window->getView();
    view.zoom(1.0f);
    whole_map_texture.loadFromFile("static/map.png");
    whole_map_texture.setSmooth(true);
}

// int renderer::create_cars(std::vector<sf::Texture*> cars_textures) {
//     for (int i = 0; i != cars_textures.size(); i++) {
//         sf::Sprite
//         players.push_back(cars_textures);
//     }
//     return 0;
// }

int renderer::create_map(size_t map_number, sf::Texture* map_texture) {
    map_sprite.setTexture(whole_map_texture);
